...
cdef real RealMatrixCallback(void* func,real val, sval n, sval m) with gil:
	cdef object o
	o=<object?>func
	return (o((val),n,m))


cdef class RealMatrix:
	cdef iMatrix[real]* mat
	...
	@staticmethod
	cdef RealMatrix _new(iMatrix[real]* mat):
		m=RealMatrix(mat.getName())
		m.mat=mat
		return m

	@staticmethod
	cdef iMatrix[real]* _getNone(RealMatrix m):
		if m:
			return m.mat
		else:
			return NULL
	...
	def subMatrix(self,str name,sval n, sval m=1,sval noff=0,sval moff=0,bint isShared=False):
		return RealMatrix._new(self.mat.subMatrix(name,n,m,noff,moff,isShared))

	def reshape(self,str name,sval n, sval m,bint isShared=False):
		return RealMatrix._new(self.mat.reshape(name,n,m,isShared))
	...
	def clone(self,str newname=None,bint isShared=False):
		if newname:
			return RealMatrix._new(self.mat.clone(newname,isShared))
		else:
			return RealMatrix._new(self.mat.clone(NULL,isShared))

	def __clone__(self):
		return self.clone()

	def __repr__(self):
		return 'RealMatrix<%s, %i x %i, %r>'%(self.getName(),self.n(),self.m(),self.isShared())

	def __reduce__(self):
		if not self.isShared():
			raise MemoryError,'Only shared memory matrices can be pickled'

		return RealMatrix,(self.getName(),self.getType(),self.mat.getSharedName(),self.mat.serializeMeta(),self.n(),self.m())
	...
	def setAt(self,real v,sval n,sval m=0):
		self.mat.setAt((v),n,m)

	def fill(self,real v):
		self.mat.fill((v))
	...
	def append(self,*args):
		...
		if isinstance(args[0],RealMatrix):
			self.mat.append(deref((<RealMatrix>args[0]).mat))
		else:
			n=self.mat.n()
			self.mat.addRows(1)
	...
	def indexOf(self,real p,sval aftern=0,sval afterm=0):
...
# extra methods for RealMatrix
	...
	def add(self,t,sval minrow=0,sval mincol=0,sval maxrow=-1,sval maxcol=-1):
		if isinstance(t,RealMatrix):
			self.mat.add(deref((<RealMatrix>t).mat),minrow,mincol,maxrow,maxcol)
		else:
	...
	def sub(self,t,sval minrow=0,sval mincol=0,sval maxrow=-1,sval maxcol=-1):
		if isinstance(t,RealMatrix):
			self.mat.sub(deref((<RealMatrix>t).mat),minrow,mincol,maxrow,maxcol)
		else:
	...
	def mul(self,t,sval minrow=0,sval mincol=0,sval maxrow=-1,sval maxcol=-1):
		if isinstance(t,RealMatrix):
			self.mat.mul(deref((<RealMatrix>t).mat),minrow,mincol,maxrow,maxcol)
		else:
	...
	def div(self,t,sval minrow=0,sval mincol=0,sval maxrow=-1,sval maxcol=-1):
		if isinstance(t,RealMatrix):
			self.mat.div(deref((<RealMatrix>t).mat),minrow,mincol,maxrow,maxcol)
		else:
