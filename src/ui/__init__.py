...
If the .ui files are not present then there should be a resource file containing them as resource items, and this is l
oaded instead. This is the case when packaged as an application using pyinstaller which will include the resource file
but not .ui files. These resource files are version dependent and generated by setup.py.

The defined modules are QtGui, QtCore, Qt, QtWidgets (which is QtGui for PyQt4), and uic. These should be loaded by modules
requiring PyQt types rather than from PyQt4/5 directly. 
'''
