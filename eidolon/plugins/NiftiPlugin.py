...
eidolon.addLibraryFile('nibabel-2.5.0.dev0-py3-none-any',False) # http://nipy.org/nibabel/index.html
...
def convertNiftiTransform(img):
    ...
    if int(hdr['qform_code'])>0:
        position=vec3(x,y,z)
        rot=rotator(-c,b,math.sqrt(max(0,1.0-(b*b+c*c+d*d))),-d)
    else:
    ...
    return position, rot, spacing, toffset, interval
    ...
    def loadObject(self,filename,name=None,imgObj=None,**kwargs):
        ...
        def _loadNiftiFile(filename,name,imgObj,task):
            with f:
                ...
                position,rot,spacing,toffset,interval=convertNiftiTransform(img)
        ...
        def _saveFile(path,obj,kwargs,task):
            with f:
                ...
                if 'datatype' in kwargs:
                    datatype=kwargs.pop('datatype') or np.float32
